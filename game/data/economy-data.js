...
const SHOP_ITEMS = [
  ...
  { id:'tourbillon_permit', qty:1, cost:0, wingCost:{ item:'silver_wing', qty:50 }, icon:'🌊', fr:'Permis Tourbillon', en:'Whirlpool Permit', desc_fr:'50× Argent'Aile requis → Îles Tourbillon (Lugia)', desc_en:'50× Silver Wing required → Whirl Islands (Lugia)' },
  { id:'carillon_permit',   qty:1, cost:0, wingCost:{ item:'rainbow_wing', qty:50 }, icon:'🔔', fr:'Permis Carillon',   en:'Bell Tower Permit', desc_fr:'50× Arcenci'Aile requis → Tour Carillon (Ho-Oh)', desc_en:'50× Rainbow Wing required → Bell Tower (Ho-Oh)' },
];

const MYSTERY_EGG_BASE_COST = 50000;
function getMysteryEggCost(stateLike) {
  const n = stateLike?.purchases?.mysteryEggCount || 0;
  return MYSTERY_EGG_BASE_COST + n * 1000;
}
...
const MYSTERY_EGG_HATCH_MS = 45 * 60 * 1000;
const POTENTIAL_MULT = [0.5, 1, 2, 5, 15];
const BASE_PRICE = { common:100, uncommon:250, rare:600, very_rare:1500, legendary:5000 };

export {
  BALLS,
  SHOP_ITEMS,
  MYSTERY_EGG_BASE_COST,
  MYSTERY_EGG_POOL,
  MYSTERY_EGG_HATCH_MS,
  POTENTIAL_MULT,
  BASE_PRICE,
  getMysteryEggCost,
};
